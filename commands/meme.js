  ...
  if (args.length !== 0) {
    ...
    if (image !== undefined) {
      ...
      if (bottomText !== undefined) {
        const memeOutput = request(`https://memegen.link/custom/${topText ? encodeURI(topText.split(" ").join("_")) : "_"}/${encodeURI(bottomText.split(" ").join("_"))}.jpg?alt=${image}&font=impact&watermark=none`);
