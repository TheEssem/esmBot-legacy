  ...
  for (let i = 0; i < messageList.length; i++) {
    if (messageList[i].attachments.array().length !== 0) {
      ...
      if (fileExtension !== "png" && fileExtension !== "jpg" && fileExtension !== "jpeg") {
        ...
        return message.reply("you need to upload a PNG or JPG file to add a bandicam watermark!");
      }
      gm(request(attachmentsList[0].url)).size((error, size) => {
        if (error) throw new Error(error);
        gm(request(attachmentsList[0].url)).composite(bandicamWatermark).gravity("North").resize(null, size.height).strip().stream((error, stdout) => {
          if (error) throw new Error(error);
          message.channel.stopTyping();
